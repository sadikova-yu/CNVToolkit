...
sub get_AD_from_info {
	my $info = shift;
	if ($info =~ /TumorVarDepth=([^;]+)/) {
		return $1;
		}
	return 0;
	}

sub get_DP_from_info {
	my $info = shift;
	if ($info =~ /TumorTotalDepth=([^;]+)/) {
		return $1;
		}
	return 0;
	}
...
sub run {
	...
	while (<GENELIST>) {
		...
		my $gene = $_;
		push @genes, $gene;
		`/home/onco-admin/bin/sga/src/SGA/sga somatic-variant-filters --tumor-bam $bam  --normal-bam $bam --reference \$hg19 $current_dir/../config/$panel/$gene.vcf.gz --annotate-only > $tmp_folder/$gene$seed.vcf 2>/dev/null`;
		}
	close GENELIST;
	...
	if ($options->{output} eq 'vaf') {
		...
		for (my $i = 0; $i < scalar @genes; $i++) {
			...
			while (<READ>) {
				...
				my @mas = split/\t/;
				if (defined($mas[6])) {
					my $ad = get_AD_from_info($mas[7]);
					my $dp = get_DP_from_info($mas[7]);
					if ($dp > 0) {
